...
//所有当前坐标系和G92偏移。
  switch (gc_block.non_modal_command) {
    case NON_MODAL_SET_COORDINATE_DATA:
      ...
      else { coord_select = gc_block.modal.coord_select; } // 将 P0 索引为当前活动坐标系
...
//如果圆圈为逆时针方向，则反转h_x2_div_d的符号（见下图）
            ...
            */
            ...
            if (delta_r > 0.005) {
              if (delta_r > 0.5) { FAIL(STATUS_GCODE_INVALID_TARGET); } //[圆弧定义误差]>0.5mm
              if (delta_r > (0.001*gc_block.values.r)) { FAIL(STATUS_GCODE_INVALID_TARGET); } //[圆弧定义误差]>0.005mm和0.1%半径
            }
          }
          break;
